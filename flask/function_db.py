...
class DataBase:
    ...
    def add_post(self, title, description, category, price, user_id, image_path=None):
        try:
            self.conn = connection_db()
            with self.conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO product (title, description, product_type_id, price, user_id, photo) "
                    "VALUES (%s, %s, %s, %s, %s, %s)",
                    (title, description, category, price, user_id, image_path)
                )
                self.conn.commit()
            return ["Ad successfully added", "success"]
        except Exception as e:
            print(f"Error: {e}")
            return None
        finally:
